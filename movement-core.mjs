...
export const fields = {startTime:['开始时间','time',''],duration:['时长','number','分钟'],lessons:['课时','number','课时'],steps:['步数','number','步'],kcal:['消耗','number','kcal'],notes:['备注','textarea','']};
...
export function validateState(s){
 ...
 for(const r of s.records){
  ...
  for(const [k,v] of Object.entries(r.values)){if(!Object.hasOwn(fields,k))fail();if(k==='notes'){if(typeof v!=='string'||v.length>5000)fail();}else if(k==='startTime'){if(typeof v!=='string'||!/^([01]\d|2[0-3]):[0-5]\d$/.test(v))fail();}else if(!numeric(v)||(k==='steps'&&!Number.isInteger(v)))fail();}
...
export function recordLabel(state,r){const t=state.types.find(t=>t.id===r.typeId);return r.name+(t?.kind==='dance'&&r.values.lessons!==undefined?` · ${r.values.lessons}课时`:'');}
